...
class TestDistributionNaTimeIndex:
    ...
    def test_null_end_drop_warns(self, state_pool_copy) -> None:
        ...
        with pytest.warns(UserWarning, match="null time index"):
            # fmt: off
            result = (
                SequenceVisualizer.distribution(bin_size="1d", allow_large=True)
                .draw(state_pool_copy, entity_feature="status")
            )
            # fmt: on
        plt.close(result.figure)
    ...
    def test_null_end_raise(self, state_pool_copy) -> None:
        ...
        with pytest.raises(ValueError, match="null time index"):
            # fmt: off
            SequenceVisualizer.distribution(bin_size="1d", na_time_index="raise", allow_large=True) \
                .draw(state_pool_copy, entity_feature="status")
            # fmt: on
...
class TestDistributionNaLabel:
    ...
    def test_null_label_raise(self, state_pool_copy) -> None:
        """na_label='raise' raises ValueError when null labels exist."""
        pool = self._pool_with_null_label(state_pool_copy)
        with pytest.raises(ValueError, match="null label"):
            # fmt: off
            SequenceVisualizer.distribution(bin_size="1d", na_label="raise", allow_large=True) \
                .draw(pool, entity_feature="nullable_cat")
            # fmt: on

    def test_null_label_drop_warns(self, state_pool_copy) -> None:
        """Default na_label='drop' emits UserWarning for null label rows."""
        pool = self._pool_with_null_label(state_pool_copy)
        with pytest.warns(UserWarning, match="null label"):
            # fmt: off
            result = (
                SequenceVisualizer.distribution(bin_size="1d", na_label="drop", allow_large=True)
                .draw(pool, entity_feature="nullable_cat")
            )
            # fmt: on
        plt.close(result.figure)

    def test_null_label_category(self, state_pool_copy) -> None:
        """na_label='category' replaces nulls with 'N/A' and renders."""
        pool = self._pool_with_null_label(state_pool_copy)
        # fmt: off
        result = (
            SequenceVisualizer.distribution(bin_size="1d", na_label="category", allow_large=True)
            .draw(pool, entity_feature="nullable_cat")
        )
        # fmt: on
        plt.close(result.figure)
