...
# ---------------------------------------------------------------------------
# Inherited T0 setter: installed by TrajectoryPool._propagate_t0()
# ---------------------------------------------------------------------------


class _InheritedT0Setter(T0Setter, register_name="_inherited"):
    """Proxy setter that delegates T0 to a parent trajectory.

    Installed on every managed sub-pool (at :meth:`_build_pools` time) and
    on every sequence built from a standalone
    :class:`~tanat.trajectory.trajectory.Trajectory`.  This ensures that
    all child objects share the trajectory-level T0 without recomputing it.

    If the parent has not computed T0 yet, :meth:`_compute_t0` triggers the
    parent's lazy computation on first access.

    The parent is held **by reference**: after ``subset()`` or a second
    ``set_t0()`` call, the parent's result is always up-to-date.

    One instance per child; each builds its own ``_df`` independently.
    """

    def __init__(self, parent: TrajectoryPool | Trajectory) -> None:
        super().__init__(settings=None)
        self._parent = parent

    @property
    def _strategy_label(self) -> str:
        # Live read: always reflects the parent's current strategy.
        # pylint: disable=protected-access
        return f"from trajectory ({self._parent._t0_setter.strategy_summary})"

    def _compute_t0(self, target: SequencePool | Sequence) -> pl.LazyFrame:
        """Return T0 from the parent trajectory's setter.

        Triggers the parent's lazy computation if it has not run yet.
        """
        id_col = target.settings.id_column
        # pylint: disable=protected-access
        parent_setter = self._parent._t0_setter
        if parent_setter.df is None:
            parent_setter.compute_from_trajectory(self._parent)
        return parent_setter.df.select([id_col, _T0]).lazy()

    def _warn_nulls(self, null_ids: list) -> None:
        # Silenced: null warnings are already emitted at trajectory level.
        pass
