...
Build an :class:`~tanat.sequence.EventSequencePool` from synthetic data
and navigate all the way down to individual entities.

An **event sequence** records punctual observations; each entity has a
single timestamp. Typical examples: sensor readings, clicks, medical visits.
...
from tanat.dataset import simulate_events, simulate_static

# %% [markdown]
# Simulate data
# ~~~~~~~~~~~~~
#
# :func:`~tanat.dataset.simulation.events.simulate_events` returns a ``pd.DataFrame``
# with columns ``id``, ``time``, and one column per named entity feature.

# %%
temporal = simulate_events(
    n_ids=50,
...
# ~~~~~~~~~~~~~~
#
# :func:`~tanat.sequence.shortcuts.build_events` infers every column that is not
# ``id`` or ``time`` as an entity feature; no explicit feature list needed.
...
# %%
print(f"ID {seq.id_value}, {len(seq)} events")
...
# :class:`~tanat.sequence.type.event.entity.EventEntity`,
# the atomic element of the pool. Positive and negative indices are both supported.

# %%
entity = seq[0]  # first event
last = seq[-1]  # last event
...
# :class:`~tanat.sequence.type.event.sequence.EventSequence` per visible ID.
# A sequence is also iterable: it yields one
# :class:`~tanat.sequence.type.event.entity.EventEntity` per row.

# %%
# Pool → Sequence : one sequence per visible ID
for seq in pool.subset(pool.unique_ids[:3]):
    print(f"  {seq.id_value}: {len(seq)} events")

# %%
# Sequence → Entity : one entity per row
seq = pool[pool.unique_ids[0]]
for entity in seq:
    print(f"  t={entity.temporal_extent}  data={entity.data()}")
